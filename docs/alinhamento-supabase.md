...
Validação local em 11/09/2026: `npm ci --ignore-scripts` passou, `npm run check` passou (TypeScript + 19 testes), `npm run build` passou com avisos existentes de bundle grande/importações mistas. Testes cobrem fórmula, paginação, retornos RPC e nomes literais de relações/RPCs; não houve gravação em produção. Não houve homologação visual/end-to-end: o navegador de verificação não está instalado neste ambiente. Revisar diff em branch separada. Não fazer merge para main antes das decisões acima e da homologação. Naquela validação, ZIP e financeiro CLT estavam fora da etapa. O ZIP foi posteriormente autorizado e implementado; ver docs/relatorio-funcionarios-zip.md. Financeiro CLT permanece fora do escopo.

## Atualização de 12/09/2026: ZIP autorizado

Relatório de Funcionários implementado na branch; detalhes e validação em [relatorio-funcionarios-zip.md](relatorio-funcionarios-zip.md). Houve somente a correção de dados explicitamente autorizada para as duas datas de desligamento ausentes, preservando as outras 14. Os relatos anteriores de nenhuma escrita no banco descrevem as etapas anteriores à autorização do ZIP.
