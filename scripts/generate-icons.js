...
const APP_ICON_PATH = path.join(PUBLIC_DIR, 'icons/icon1.png');
const FAVICON_PATH = path.join(PUBLIC_DIR, 'icons/icon1.png');
...
if (!fs.existsSync(APP_ICON_PATH)) {
  console.error("❌ icons/icon1.png not found. Please upload it first.");
  process.exit(1);
}
...
async function generate() {
  console.log("Generating icons from icon1.png...");
  
  const appImage = sharp(APP_ICON_PATH);
  const faviconImage = sharp(FAVICON_PATH);
  
  // 1. Generate Favicons from icon1.png
  ...
  // 2. Generate PWA App Icons from icon1.png
